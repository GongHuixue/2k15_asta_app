...
public:
	...
	virtual int SimplePmtMain(void)= 0;
	virtual int SimplePmtAux(void)= 0;
	virtual int SimplepmtFreesatTunnelledInfo(void)=0;
};
...
public: \
    ...
    InterfaceParam<Comp,int> m_SimplePmtMain; \
    InterfaceParam<Comp,int> m_SimplePmtAux; \
     InterfaceParam<Comp,int> m_SimplepmtFreesatTunnelledInfo; \
    Comp ## _ ## intf( Comp *parent):m_VcTableMain(parent),m_VcTableAux(parent),m_MgtTableMain(parent),m_MgtTableAux(parent),m_PmtTableMain(parent),m_PmtTableAux(parent),m_BasicPidInfoMain(parent),m_BasicPidInfoAux(parent),m_AudioInfoMain(parent),m_AudioInfoAux(parent),m_SubtitlingInfoMain(parent),m_SubtitlingInfoAux(parent),m_TeletextInfoMain(parent),m_TeletextInfoAux(parent),m_ComponentTagInfoMain(parent),m_ComponentTagInfoAux(parent),m_ReqTableMain(parent),m_ReqTableAux(parent),m_EitTableMain(parent),m_EitTableAux(parent),m_SimplePmtMain(parent),m_SimplePmtAux(parent),m_SimplepmtFreesatTunnelledInfo(parent) \
    {\
        m_parent = parent;\
    }\
...
virtual int SimplePmtMain(void){ return m_SimplePmtMain.Call();}\
virtual int SimplePmtAux(void){ return m_SimplePmtAux.Call();}\
virtual int SimplepmtFreesatTunnelledInfo(void){ return m_SimplepmtFreesatTunnelledInfo.Call();}\
};\
