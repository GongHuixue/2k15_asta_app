 ...
 *
 *          %name: hsveuins_mfinsort.c %
 *       %version: TVG_FUSION_14 %
...
#include "_hsveuins_mfinsort.h"

TRACESYMBOL(m, "dvb/installation/hsveuins_mfinsort_m")
...
/*****   GLOBAL VARIABLES   *****/
Pump CHsvEuropeInstallation_mfinsort_Priv::s__mfinsortPmp;

int CHsvEuropeInstallation_mfinsort_Priv::s__mFirstInstalledPreset = 0xFFFF,CHsvEuropeInstallation_mfinsort_Priv::s__mCurrentTvPreset = INVALID_PRESET,CHsvEuropeInstallation_mfinsort_Priv::s__mCurrentRadioPreset = INVALID_PRESET;


/*****   Local Structures and enums   *****/


int CHsvEuropeInstallation_mfinsort_Priv::s__mServicePriority[MAX_SERVICE_TYPES] =
{
...
void CHsvEuropeInstallation_mfinsort_Priv::mFinSortHandler(int value, Nat32 param)
{
    UNUSED(param);
...
void CHsvEuropeInstallation_mfinsort_Priv::mNoSorting(int evCurrentSortStep)
{
    ...
    for(dbindex = 0; dbindex < noOfAnaRecs; ++dbindex)
    {
        ...
        if(pgdb_GetByIndex(ANALOG_TABLE, dbindex, (Address)&anadat))
        {
            if( (TRUE == anadat.Tuned) &&
                    (ifinsort_IsPresetInstalled(anadat.Frequency) == FALSE) )
            {
                ++mCurrentTvPreset;
                presetdat.PresetNumber = mCurrentTvPreset;
    ...
    pmp_PmpSend(mfinsortPmp, evStepComplete, (Nat32)NULL);
}

void CHsvEuropeInstallation_mfinsort_Priv::mSortHonouredServices(int evCurrentSortStep)
{
    ...
    svcdat.OriginalNetworkId =   ifinsort_GetOriginalNetworkId(sysset_GetSystemCountry());
    ...
    pmp_PmpSend(mfinsortPmp, (evCurrentSortStep + 1), (Nat32)NULL);
}


Bool CHsvEuropeInstallation_mfinsort_Priv::mDigitalPresetEntryforValidLCN(HsvPgdatDigServiceData *svcdat)
{
	Bool retval = FALSE;
	int  presetnum = -1;
	Nat8 i=0;
	...
	{
		if( PRESET_NOT_ALLOCATED(svcdat->MultiplePreset[i]) && VALID_LCN_RANGE(svcdat->MultipleLCN[i]) )
		{
			
			presetnum = ASSIGN_PRESET_NUMBER(svcdat->MultipleLCN[i], svcdat->ServiceType);
			TraceNotice (m, "PresetNumber allocated %d i %d MultipleLCN %d", presetnum, i, svcdat->MultipleLCN[i]);
			if( mIsPresetOccupied(presetnum) == FALSE )
			{
				mAddOrUpdateEntryToPresetTable(svcdat, presetnum, ADD_ENTRY,i);
				retval = TRUE;
			}
			else
			{
				retval = mUpdateDigitalPresetEntryforValidLCN(svcdat,i);
			}
		}
	}
	return retval;
...
Bool CHsvEuropeInstallation_mfinsort_Priv::mUpdateDigitalPresetEntryforValidLCN(HsvPgdatDigServiceData *svcdat,Nat8 i)
{
    ...
    presetdat.PresetNumber = ASSIGN_PRESET_NUMBER(svcdat->MultipleLCN[i], svcdat->ServiceType);
    if(pgdb_GetByKey(PRESET_TABLE, (Address)&presetdat) == TRUE)
	{
		...
		{                        
            ...
            {
                INVALIDATE(svcdat->MultipleLCN[i]); 
				pgdb_Update(SERVICE_TABLE, (Address)svcdat);				
				CanBeReplaced = FALSE;    
			}	
		...
		if(Proceed)
		{
			...
			if(PresetRequireRefilling)
			{
                ...
                {					
					...
					{
						if(presetdat.PresetNumber == oldsvcdat.MultiplePreset[k])
						{
							INVALIDATE(oldsvcdat.MultipleLCN[k]);
							oldsvcdat.MultiplePreset[k] = INVALID_PRESET;
							break;
						}
					}
				...
				mAddOrUpdateEntryToPresetTable(svcdat, ASSIGN_PRESET_NUMBER(svcdat->MultipleLCN[i], svcdat->ServiceType), UPDATE_ENTRY,i);
				retval = TRUE;
			}
			else 
			{
...
void CHsvEuropeInstallation_mfinsort_Priv::mSortNonHonouredServices(int evCurrentSortStep, Bool CheckForCurrentOnId)
{
    ...
    for( i = from; retval && (i <= to) && (pref_index_found == TRUE); i++)
    {
        ...
        for(dbindex = from; dbindex <= to; dbindex++)
        {
            ...
            if(pgdb_GetByIndex(SERVICE_TABLE, dbindex, (Address)&svcdat) == TRUE)
            {
				...
				if(mPresetAllocated(&svcdat)== FALSE)
				{
...
Bool CHsvEuropeInstallation_mfinsort_Priv::mPresetAllocated(HsvPgdatDigServiceData *svcdat) 
{
	Bool retval = FALSE;
	Nat8 i =0;
	...
	for(i=0;i<MAX_LCNS;i++)
	{
		TraceNotice (m, " i %d MultiplePreset %d ", i, svcdat->MultiplePreset[i]);
		/* ANEESH: TODO: Remove this check. SQLITE is storing 0xFFFF string as 0xFFFD. Need more investigation 
			on this */
		if ( (svcdat->MultiplePreset[i] != INVALID_PRESET) && (svcdat->MultiplePreset[i] != 0xFFFD) )
		{
			retval = TRUE;
			break;
		}
...
void CHsvEuropeInstallation_mfinsort_Priv::mDigitalPresetEntryforInValidLCN(int dbindex)
{
    int CurrentPreset;
    HsvPgdatDigServiceData svcdat;
    ...
    if(TRUE == pgdb_GetByIndex (SERVICE_TABLE, dbindex, (Address)&svcdat))
    {
        ...
        {
			mAddOrUpdateEntryToPresetTable(&svcdat, CURRENT_PRESET(svcdat.ServiceType), ADD_ENTRY,0);
            if(IS_RADIO_SERVICE(svcdat.ServiceType))
            {
                mCurrentRadioPreset++;
            }
...
void CHsvEuropeInstallation_mfinsort_Priv::mAddOrUpdateEntryToPresetTable(HsvPgdatDigServiceData *svcdat, int PresetNo, Bool AddOrUpdate,Nat8 index)
{
    ...
    svcdat->EPGEnabled = FALSE; 
    svcdat->UserModifiedName = FALSE;                 
	presetdat.FavoriteNumber = svcdat->MultipleFavorite[index];
    ...
    svcdat->MultiplePreset[index] = presetdat.PresetNumber;
	svcdat->MultipleFavorite[index] = 0; /* Reset the flag */
...
void CHsvEuropeInstallation_mfinsort_Priv::mSortAnalogServices(int evCurrentSortStep)
{
...
inline void CHsvEuropeInstallation_mfinsort_Priv::mFindPresetNumberForService(int AnalogOrDigital,int ServiceType)
{
...
inline int CHsvEuropeInstallation_mfinsort_Priv::mPlaceNonHonouredOrAnalogServices(int AnalogOrDigital,int ServiceType)
{
...
void CHsvEuropeInstallation_mfinsort_Priv::mReInnovateFromPreset(int ServiceType, int *FromPreset)
{
    int numdigservice;
...
inline int CHsvEuropeInstallation_mfinsort_Priv::mLastLCNBasedPreset(Bool CheckForRadioServices)
{
            ...
            if(presetdata.Type == HsvOnePart)
            {
                ...
                if(pgdb_GetByKey(SERVICE_TABLE, (Address)&svcdat))
                {
                    if(IS_RADIO_SERVICE(svcdat.ServiceType) == CheckForRadioServices)
                    {
						...
						{
							if( (VALID_LCN_RANGE(svcdat.MultipleLCN[j])) && 
								(ASSIGN_PRESET_NUMBER(svcdat.MultipleLCN[j], svcdat.ServiceType) == svcdat.MultiplePreset[j]) && 
								(presetdata.PresetNumber == svcdat.MultiplePreset[j]) )
							{
								retval = svcdat.MultiplePreset[j];
								proceed = FALSE;
								break;
							}
						}
						if((proceed == FALSE) && (j < MAX_LCNS))
						{
...
int CHsvEuropeInstallation_mfinsort_Priv::mLargestGapInForwardRange(int FromPreset, int ToPreset)
{
...
int CHsvEuropeInstallation_mfinsort_Priv::mCompareServices(HsvPgdatDigServiceData *svcdata, int pref_index)
{
    int retval = 0;
    ...
    if(pgdb_GetByIndex(SERVICE_TABLE, pref_index, (Address)&pref_svcdata))
    {
		...
		retval = (retval == 0)? (svcdata->MultipleLCN[0] - pref_svcdata.MultipleLCN[0]):retval;   
...
int CHsvEuropeInstallation_mfinsort_Priv::mStrCmp( Nat16 * str1, Nat16 * str2 )
{
    /*int ret = 0;
...
int CHsvEuropeInstallation_mfinsort_Priv::mGetServicePriority(int serviceType)
{
    int i = 0, retval = 1;
    Bool found = FALSE;
...
Bool CHsvEuropeInstallation_mfinsort_Priv::mIsPresetOccupied(int PresetNumber)
{
    Bool retval = FALSE;
    HsvPgdatPresetData presetdat;
...
void CHsvEuropeInstallation_mfinsort_Priv::ifinsortpow_Init()
{
    mfinsortPmp = pmp_PmpCreateEx( pen_PumpEngine, mFinSortHandler, FALSE, "hsveuins_mfinsort" );
    
    mFirstInstalledPreset = 0xFFFF;
    
}

void CHsvEuropeInstallation_mfinsort_Priv::ifinsortpow_TurnOn()
{

}

void CHsvEuropeInstallation_mfinsort_Priv::ifinsortpow_TurnOff()
{

}
void CHsvEuropeInstallation_mfinsort_Priv::ifinsort_Initialise(void)
{
    if(AUTO_INSTALL)
    {
...
void CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AddDigitalPreset (HsvPgdatDigServiceData *psvcdat)
{
    int dbindex = -1;
    Bool retval = FALSE;
...
Bool CHsvEuropeInstallation_mfinsort_Priv::mValidLCN(HsvPgdatDigServiceData *psvcdat)
{
	Bool retval = FALSE;
	Nat8 i =0;
	for(i=0;i<MAX_LCNS;i++)
	{
		if(VALID_LCN_RANGE(psvcdat->MultipleLCN[i]))
		{
			retval = TRUE;
			break;
		}
...
void CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AddAnalogPreset(Address adata)
{
...
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AutoStoreManualInstallationValues ()
{   
    Bool retval = FALSE;
...
int CHsvEuropeInstallation_mfinsort_Priv::ifinsort_GetNumberOfAnalogChannelsFound()      
{
    int retval = 0;
...
void CHsvEuropeInstallation_mfinsort_Priv::ifinsort_UpdateDigitalPreset(HsvPgdatDigServiceData *psvcdat)
{
    HsvPgdatPresetData presetdat;
	Nat8 i=0;
    ...
    if(TRUE == pgdb_GetByKey(SERVICE_TABLE,(Address)psvcdat))
    {
        for(i=0;i<MAX_LCNS;i++)
		{
			presetdat.PresetNumber = psvcdat->MultiplePreset[i];
			presetdat.Type = HsvOnePart;
...
int CHsvEuropeInstallation_mfinsort_Priv::ifinsort_GetOriginalNetworkId(int CurrentCountry)
{
    Nat16 ONWID   = 0xFFFF;
...
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_IsPresetInstalled(Int32 frequency)
{
    Bool retval = FALSE;
...
void CHsvEuropeInstallation_mfinsort_Priv::ifinsort_SortAll()
{
    TraceDebug(m,"%s %d mfinsortPmp %d", __FUNCTION__, __LINE__, mfinsortPmp);  
    pmp_PmpSend(mfinsortPmp, evStep1, (Nat32)NULL); 
}

HsvChannel CHsvEuropeInstallation_mfinsort_Priv::ifinsort_GetInstalledWeakSignalChannel()
{
    HsvChannel channel;
...
int CHsvEuropeInstallation_mfinsort_Priv::mGetAutomaticPlatformPresetNumber(void)
{
...
Bool CHsvEuropeInstallation_mfinsort_Priv::mServiceIsOfPreviousPass(HsvPgdatPresetData presetdat)
{
    Bool retval = FALSE;
    HsvPgdatDigServiceData svcdat;
...
int CHsvEuropeInstallation_mfinsort_Priv::mConvertToDVBServiceType(int servicetype)
{
...
Bool CHsvEuropeInstallation_mfinsort_Priv::mCompareMultiplex(HsvPgdatDigTSData oldtsdata, HsvPgdatDigTSData tsdata)
{
    Bool retval     =   TRUE;    
...
Bool CHsvEuropeInstallation_mfinsort_Priv::mIsCurrentPresetInvalid( int PresetNo, int ServiceType)
{
    HsvPgdatPresetData presetdata;
...
Bool CHsvEuropeInstallation_mfinsort_Priv::mValidPresetForService(int presetnum, int svcType)
{
...
int CHsvEuropeInstallation_mfinsort_Priv::mAnalogStartPreset(void)
{
...
int CHsvEuropeInstallation_mfinsort_Priv::mNonRadioStartPreset(void)
{
    int retval, LastLCNPreset = mLastLCNBasedPreset(FALSE);
...
int CHsvEuropeInstallation_mfinsort_Priv::mRadioStartPreset(void)
{
...
void CHsvEuropeInstallation_mfinsort_Priv::mGetPresetRangeForService(int svcType, int *startpreset, int *endpreset)
{
...
/*Stub */
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AssignPresetsForCurrentCountryNonLCNs(int CurrentOnwid, int *FillHoles)
{
	return 0;
}
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AnalogPresetEntry(void)
{
	return 0;
}
void CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AssignPresetsForCountry(int ONID)
{
	return;
}
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_DeleteAllDigitalChannels()
{
	return 0;
}    
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_DeleteInvalidPTCTable()
{
	return 0;
}
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AssignPresetsForCurrentCountry  (int CurrentOnwid, Bool CheckLCN)
{
	return 0;
}
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AssignPresetsForCurrentCountryLowPrioLCN(int CurrentOnwid)
{
	return 0;
}
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AssignPresetsWithStaticTable ( int CurrentOnwid, int Country )
{
	return 0;
}
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_UpdatingLCNDuringUpdateInstallation()
{
	return 0;
}
Bool CHsvEuropeInstallation_mfinsort_Priv::ifinsort_AssignPresetsOfOtherCountries   (void)
{
	return 0;
}
void CHsvEuropeInstallation_mfinsort_Priv::ifinsort_RemoveDigitalPresets(void)
{
	return;
}
