 ...
 *
 *          %name: hsvdvbsrussia_mSort.c %
 *       %version:  TVG_4 %
 * %date_modified:  %
 *    %derived_by:  shreeshav % 
 */

#include "_hsvdvbsrussia_mSort.h"

#include "russiasort.h"

TRACESYMBOL(m, "dvb/installation/hsvdvbsrussia/sort")
/****************************************************************************
 * Macros and types
 ****************************************************************************/
...
int CHsvDvbsRussia_mSort_Priv::s__mCurrentRadioIndex = 0,CHsvDvbsRussia_mSort_Priv::s__mCurrentPresetIndex = 0;

Pump CHsvDvbsRussia_mSort_Priv::s__Sortingpmp;
...
void CHsvDvbsRussia_mSort_Priv::isortpow_Init()
{
	PrintfNotice ("Sort Pow Init being called \n");
	Sortingpmp = pmp_PmpCreateEx(pen_PumpEngine, SortingHandler, TRUE, "hsvdvbinsRussia_msort");   
}

void CHsvDvbsRussia_mSort_Priv::isortpow_TurnOn (void)
{

}

void CHsvDvbsRussia_mSort_Priv::isortpow_TurnOff (void)
{
...
void CHsvDvbsRussia_mSort_Priv::irussiasort_Sort (Nat32 PackageIndex)
{
...
void CHsvDvbsRussia_mSort_Priv::SortingHandler( int event, Nat32 startindex )
{
...
void CHsvDvbsRussia_mSort_Priv::mSortHonouredServices(Bool CheckForRadioServices, int startindex)
{
...
void CHsvDvbsRussia_mSort_Priv::mSortNonHonouredServices(Bool CheckForRadioServices, int startindex)
{
    HsvPgdatDigServiceData svcdat;
    int i = 0, NoOfRecs = pgdb_GetNoRecords(SERVICE_TABLE);
	int nextindex =0;
...
void CHsvDvbsRussia_mSort_Priv::mAddEntryToPresetTable(HsvPgdatDigServiceData *svcdat, int PresetNo)
{
...
void CHsvDvbsRussia_mSort_Priv::mDigitalPresetEntryforValidLCN(int dbindex)
{
    HsvPgdatDigServiceData svcdat;
    HsvPgdatPresetData presetdat;
    ...
    if( pgdb_GetByIndex(SERVICE_TABLE, dbindex, (Address)&svcdat) == TRUE )
    {
		...
		if(mPriorityCheck(OldSvcdat.ServiceType, svcdat.ServiceType))
		{
			...
			TraceNotice(m,"S2App: Invalidating the LCN %d",OldSvcdat.LCN);
...
Bool CHsvDvbsRussia_mSort_Priv::mPriorityCheck(Nat8 OldServiceType, Nat8 NewServiceType)
{
	Bool retval = FALSE;
	...
	switch(OldServiceType)
	{
		...
		case AppApiAdvCodecRadioService:
			if(NewServiceType!= AppApiRadioService)
			{
				retval = TRUE;
			}

		case AppApiTelevisionService:
			if(( NewServiceType == AppApiAdvCodecSDTelevisionService) ||  (NewServiceType== AppApiAdvCodecHDTelevisionService))
			{
				retval = TRUE;
			}

		case AppApiAdvCodecSDTelevisionService:
			if(NewServiceType == AppApiAdvCodecHDTelevisionService)
			{
				retval = TRUE;
			}
		default :
...
void CHsvDvbsRussia_mSort_Priv::mDigitalPresetEntryforInValidLCN(int dbindex)
{
...
int CHsvDvbsRussia_mSort_Priv::mGetPresetNoForService(HsvPgdatDigServiceData svcdat, int FromPreset, int ToPreset)
{
    int CurrentPresetIndex = 0;
    HsvPgdatPresetData presetdat;
...
int CHsvDvbsRussia_mSort_Priv::mFirstAvailableGapInPresetList(int FromPreset, int ToPreset)
{
