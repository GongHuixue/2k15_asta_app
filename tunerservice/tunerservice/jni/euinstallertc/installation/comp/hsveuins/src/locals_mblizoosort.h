...
void mSwap(int* a, int* b);
 void mMaxHeapify( struct MaxHeap* maxHeap, int idx);
struct  MaxHeap* mCreateAndBuildHeap(struct BlizooSort *array, int size);
 void mHeapSort(struct BlizooSort* array, int size);
void mBlizoosortHandler(int value, Nat32 param);
void mNoSorting(int evCurrentSortStep);
void mSortAnalogServices(int evCurrentSortStep);
void mSortHonouredServices(Bool CheckForRadioServices, int evCurrentSortStep);
void mSortNonHonouredServices(Bool CheckForRadioServices, int evCurrentSortStep);
void mSortNonHonouredServicesRCSRDS(int evCurrentSortStep);
 void mSortBlizooServices( int evCurrentSortStep);
