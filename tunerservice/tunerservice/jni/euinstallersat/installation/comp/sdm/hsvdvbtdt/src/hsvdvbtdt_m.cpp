...
void CHsvDvbTDT_Priv::pow_Init(void)
{
    ...
    {        
        ...
        DmxData[dmx].SectionInfo[0] = conf_TdtParsing;
...
void CHsvDvbTDT_Priv::ena_Enable(HsvDemux dmx)
{
    ...
    InitDateTime(dmx);
    /* Initialise Sections */
	//TraceNotice(m,"Entered into ena_enable for TDT\n");
    if(conf_IsActionAllowed(dmx, DmxData[dmx].SectionInfo[TDT_INDEX]))
    {
//    	TraceNotice(m,"Enabled TDT section acquisition");
        DmxData[dmx].dt.Version =   HSV_INVALID_VERSION;
        DmxData[dmx].SubscrId  = sec_RequestSection(HSV_MAP_DMX_TO_DEST(dmx), (int)TDT_PID_VALUE, (int)TDT_TABLE_ID,(int)(&DmxData[dmx].SubscrId ), FNADDR(Tdt_Parser_Parser));
...
Bool CHsvDvbTDT_Priv::PopulateTimeDate ( HsvDemux dmx, TypeTdt_Parser* TimeDateTable, int Ppty, int tableid, int code)
{
    ...
    if( sec_SctArrived & code )
    {
        if(TimeDateTable->TableId == TDT_TABLE_ID)
        {
			...
			datetime = DmxData[dmx].dt.dt;
			version = DmxData[dmx].dt.Version;
			//strapiN_OnAdded(HSV_MAP_DMX_TO_DEST(dmx),HSVSPID(DateTimeData, dvb, tdt),0,0,0);
			if( ctrlN_iPresent())
			{
				ctrlN_OnEvent( dmx, ctrlN_DataAvailable, ctrlN_TDT);
			}
        }
    }
