/**********************************************************************
 *
 *   CPP Source:       hsveuins_mblizoosort.cpp
 *   Instance:       blr81mgr_2
 *   Description:
 *   %created_by:    ponraman %
 ...
 *
 **********************************************************************/
#include "_hsveuins_mblizoosort.h"

TRACESYMBOL(m, "dvb/installation/hsveuins_mblizoosort")

#include "hsveuins_blizoosort.h"

/*****************************************
  provides IHsvHybridPresetSort blizoosort
 ******************************************/


void CHsvEuropeInstallation_mblizoosort_Priv::iblizoopow_Init()
{
	mBlizoosortPump = pmp_PmpCreateEx( pen_PumpEngine, mBlizoosortHandler, FALSE, "hsveuins_mblizoosort" );
}

void CHsvEuropeInstallation_mblizoosort_Priv::iblizoopow_TurnOn()
{
}

void CHsvEuropeInstallation_mblizoosort_Priv::iblizoopow_TurnOff()
{
}

void CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_Initialise()
{
	if(AUTO_INSTALL)
	{
...
void CHsvEuropeInstallation_mblizoosort_Priv :: mSwap(int* a, int* b)
{
	int t = *a;
	*a = *b;
	*b = t;
...
void  CHsvEuropeInstallation_mblizoosort_Priv :: mMaxHeapify( MaxHeap* maxHeap, int idx)
{
	int largest = idx;
	int left = (idx << 1) + 1;
	int right = (idx + 1) << 1;
	...
	if (left < maxHeap->size &&  maxHeap->array[left].serviceid > maxHeap->array[largest].serviceid)
		largest = left;

	if (right < maxHeap->size && maxHeap->array[right].serviceid > maxHeap->array[largest].serviceid)
		largest = right;

	if (largest != idx)
	{
		mSwap(&maxHeap->array[largest].serviceid, &maxHeap->array[idx].serviceid);
		mSwap(&maxHeap->array[largest].index, &maxHeap->array[idx].index);
		mMaxHeapify(maxHeap, largest);
	}
}

MaxHeap*  CHsvEuropeInstallation_mblizoosort_Priv :: mCreateAndBuildHeap(BlizooSort *array, int size)
{
	int i;
	...
	for (i = (maxHeap->size - 2) / 2; i >= 0; --i)
		mMaxHeapify(maxHeap, i);

	return maxHeap;
}

void  CHsvEuropeInstallation_mblizoosort_Priv :: mHeapSort(BlizooSort* array, int size)
{
	struct MaxHeap* maxHeap = mCreateAndBuildHeap(array, size);

	while (maxHeap->size > 1)
	{
		mSwap(&maxHeap->array[0].serviceid, &maxHeap->array[maxHeap->size - 1].serviceid);
		mSwap(&maxHeap->array[0].index, &maxHeap->array[maxHeap->size - 1].index);
		--maxHeap->size;
		mMaxHeapify(maxHeap, 0);
	}

	free(maxHeap);
}

void CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AddAnalogPreset(Address adata)
{
...
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AutoStoreManualInstallationValues()
{
	Bool retval = FALSE;
	int CurrentPreset;
	HsvPgdatAnalogData    anadat,tempdat;

	int i, status, noOfRecs = pgdb_GetNoRecords(ANALOG_TABLE);
	if( noOfRecs == 0)
	{
		mCurrentTvPreset = ANALOG_STARTPRESET();
	}
	else if(pgdb_GetByIndex(ANALOG_TABLE, 0, (Address)&tempdat))
	{
		mCurrentTvPreset = ++(tempdat.PresetNumber);
	}
...
void CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_RemoveDigitalPresets()
{
...
int CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_GetNumberOfAnalogChannelsFound()
{
	int retval = 0;
	...
	TraceDebug(m,"%s %d\ retval = %d n",__FUNCTION__, __LINE__,retval);
	return retval;
}

void CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_UpdateDigitalPreset(HsvPgdatDigServiceData *psvcdat)
{
...
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_DeleteInvalidPTCTable()
{
...
void CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_SortAll()
{
	int   event     = 0;

	pmp_PmpSend(mBlizoosortPump, event, (Nat32)NULL);
}

Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_IsPresetInstalled(Int32 frequency)
{
	Bool retval = FALSE;
...
HsvChannel CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_GetInstalledWeakSignalChannel()
{
	HsvChannel channel;
...
void CHsvEuropeInstallation_mblizoosort_Priv::mBlizoosortHandler(int value, Nat32 param)
{
	UNUSED(param);
	...
	switch(value)
	{
	case evServiceIDSorting:
		TraceNotice(m,"Calling mSortBlizooServices");
		mSortBlizooServices(value);
		break;
	case evAnalogSorting:
		TraceDebug(m,"Calling mSortAnalogServices\n");
		mSortAnalogServices(value);
		break;
	case evComplete:
		iblizoosort_DeleteInvalidPTCTable();
...
void CHsvEuropeInstallation_mblizoosort_Priv::mNoSorting(int evCurrentSortStep)
{
    ...
    for(dbindex = 0; dbindex < noOfPresetRecs; ++dbindex)
    {
        ...
        {
        	if(presetdat.PresetNumber > mCurrentBlizooPreset)
        		mCurrentBlizooPreset = presetdat.PresetNumber;
        }
    }
    ...
    for(dbindex = 0; dbindex < noOfDigRecs; ++dbindex)
    {
        ...
        if(pgdb_GetByIndex(SERVICE_TABLE, dbindex, (Address)&svcdat) == TRUE)
        {
        	...
        	{
        		++mCurrentBlizooPreset;
        		presetdat.PresetNumber = mCurrentBlizooPreset;
    ...
    for(dbindex = 0; dbindex < noOfAnaRecs; ++dbindex)
    {
        ...
        if(pgdb_GetByIndex(ANALOG_TABLE, dbindex, (Address)&anadat))
        {
            if( (anadat.Tuned == TRUE) &&
                    (iblizoosort_IsPresetInstalled(anadat.Frequency) == FALSE) )
            {
                ++mCurrentBlizooPreset;
                presetdat.PresetNumber = mCurrentBlizooPreset;
    ...
    mCurrentTvPreset = mCurrentBlizooPreset;

    pmp_PmpSend(mBlizoosortPump, evComplete, (Nat32)NULL);
}

void CHsvEuropeInstallation_mblizoosort_Priv::mSortAnalogServices(int evCurrentSortStep)
{
	HsvPgdatAnalogData anadat;
	...
	int NetworkOperator = rins_GetAttribute(mode,rins_AttributeNetworkOperator);
	...
	if(rins_InstallationModeBackground != rins_GetInstallationMode())
	{
		...
		for(i = (noOfRecs - 1); i >= 0; i--)
		{
			if(pgdb_GetByIndex(ANALOG_TABLE, i, (Address)&anadat))
			{
				...
				if( (anadat.Tuned == TRUE) && (iblizoosort_IsPresetInstalled(anadat.Frequency) == FALSE) )
				{
	...
	pmp_PmpSend(mBlizoosortPump, (evCurrentSortStep + 1), (Nat32)NULL);

}
void CHsvEuropeInstallation_mblizoosort_Priv::mSortBlizooServices( int evCurrentSortStep)
{
	HsvPgdatDigServiceData svcdat;
	HsvPgdatPresetData presetdat;
	int index = 0,valid_index=0, NoOfRecs = pgdb_GetNoRecords(SERVICE_TABLE);
	BlizooSort *array  =new BlizooSort [NoOfRecs];
	mCurrentBlizooPreset = 0;
	bool retVal = false;

	for (index =0; index < NoOfRecs;index++)
	{
		retVal =  pgdb_GetByIndex(PRESET_TABLE, index, (Address)&presetdat);
		if((retVal) && (presetdat.Type == HsvOnePart))
		{
			mCurrentBlizooPreset = presetdat.PresetNumber;
			break;
		}
	...
	for( index = 0; index < NoOfRecs; index++)
	{
		if( pgdb_GetByIndex(SERVICE_TABLE, index, (Address)&svcdat) == TRUE  &&  PRESET_NOT_ALLOCATED(svcdat) )
		{
			array[valid_index].index = index;
			array[valid_index].serviceid =  svcdat.ServiceId;
			valid_index++;
			TraceDebug(m,"%s %d valid_index = %d\n", __FUNCTION__,__LINE__,valid_index);
		}
	}

	mHeapSort(array,valid_index);

	for( index = 0; index < valid_index;index++)
	{
		if( pgdb_GetByIndex(SERVICE_TABLE,array[index].index, (Address)&svcdat) == TRUE )
		{
			mCurrentBlizooPreset++;
			presetdat.PresetNumber =  mCurrentBlizooPreset;
			presetdat.Type = HsvOnePart;
			ASSIGN_PRESET_FROM_SVC(presetdat,svcdat);
			pgdb_Add(PRESET_TABLE, (Address)&presetdat);
			svcdat.PresetNumber = mCurrentBlizooPreset;
			pgdb_Update(SERVICE_TABLE,(Address) &svcdat);
		}
	}
	delete [] array;
...
void CHsvEuropeInstallation_mblizoosort_Priv::mDigitalPresetEntryforInValidLCN(int dbindex)
{
	int CurrentPreset;
	HsvPgdatDigServiceData svcdat;
	Bool retVal = 0;
...
int CHsvEuropeInstallation_mblizoosort_Priv::mAddPresetEntryForInstalledAnalog(HsvPgdatAnalogData anadat, int PresetNumber)
{
...
Bool CHsvEuropeInstallation_mblizoosort_Priv::mAddPresetEntryForNewAnalog(HsvPgdatAnalogData *anadat, int PresetNumber)
{
	Bool retval = TRUE;
	HsvChannel channel;
...
int CHsvEuropeInstallation_mblizoosort_Priv::mCompareServices(HsvPgdatDigServiceData *svcdata, int pref_index)
{
	int i = 0, retval = 0, sortkey[6] = { div_CurrentCountryInvalidLCNSortKey1, div_CurrentCountryInvalidLCNSortKey2, div_CurrentCountryInvalidLCNSortKey3,
			div_CurrentCountryInvalidLCNSortKey4, div_CurrentCountryInvalidLCNSortKey5, div_KeyInvalid };
...
int CHsvEuropeInstallation_mblizoosort_Priv::mStrCmp(Nat16 *str1, Nat16 *str2)
{
	int ret = 0;
...
int CHsvEuropeInstallation_mblizoosort_Priv::mGetServicePriority(int serviceType)
{
	int i = 0, retval = 1;
	Bool found = FALSE;
...
void CHsvEuropeInstallation_mblizoosort_Priv::mReadCableDS(void)
{
...
void CHsvEuropeInstallation_mblizoosort_Priv::mGetFileName(char *FileName, char *FilePath)
{
...
void CHsvEuropeInstallation_mblizoosort_Priv::mDeleteTSByIndex(int index)
{
	HsvPgdatDigTSData tsdat;
	HsvPgdatDigServiceData svcdat;
	HsvPgdatPresetData presetdat;
...
void CHsvEuropeInstallation_mblizoosort_Priv::mAssignPresetsFromStaticTable(Bool CheckForRadioServices, int evCurrentSortStep)
{
	HsvPgdatDigServiceData svcdat;
	...
	{
		NoOfRankingSvcs = sizeof(mGermanyKDGRankingList)/sizeof(mGermanyKDGRankingList[0]);
		RankingListOrder = mGermanyKDGRankingList;
	}
	else
	{
	...
	int event = 0;//(evCurrentSortStep == evStaticSortingStep2)? (evCommonStep1): (evCurrentSortStep + 1);
	TraceDebug(m,"Pmp send done with event = %d\r\n", event);
	pmp_PmpSend(mBlizoosortPump, event, (Nat32)NULL);
}

void CHsvEuropeInstallation_mblizoosort_Priv::mAddOrUpdateEntryToPresetTable(HsvPgdatDigServiceData *svcdat, int PresetNo, Bool AddOrUpdate)
{
	HsvPgdatPresetData presetdat, tmppresetdat;
...
Bool CHsvEuropeInstallation_mblizoosort_Priv::mIsPresetReplacable(HsvPgdatPresetData presetdat, HsvPgdatDigServiceData svcdat)
{
	Bool retval = FALSE;
	...
	if( (div_UpdateLCN(rins_Cable) == TRUE) && !MANUAL_INSTALL )
	{
		...
		if( pgdb_GetByKey (SERVICE_TABLE,(Address)&oldsvcdat) == TRUE)
		{
			...
			if( (!VALID_LCN_RANGE(oldsvcdat.LCN)))
			{
				retval = TRUE;
			}
		}
...
Bool CHsvEuropeInstallation_mblizoosort_Priv::mIsCurrentPresetInvalid( int PresetNo, int ServiceType)
{
...
inline void CHsvEuropeInstallation_mblizoosort_Priv::mFindPresetNumberForService(int ServiceType)
{
...
void CHsvEuropeInstallation_mblizoosort_Priv::mAssignPresetNumberForService(int ServiceType)
{
	...
	{
		mCurrentTvPreset = (NetworkOperator == rins_Blizoo && HsvAnalog != temp.Type) ? ((temp.PresetNumber > 1001)?temp.PresetNumber + 1 : 1001 ): mLargestGapInForwardRange(FromPreset, ToPreset);
	}
...
inline int CHsvEuropeInstallation_mblizoosort_Priv::mLastLCNBasedPreset(Bool CheckForRadioServices)
{
...
int CHsvEuropeInstallation_mblizoosort_Priv::mLargestGapInForwardRange(int FromPreset, int ToPreset)
{
...
Bool CHsvEuropeInstallation_mblizoosort_Priv::mValidPresetForService(int presetnum, int svcType)
{
...
void CHsvEuropeInstallation_mblizoosort_Priv::mGetPresetRangeForService(int svcType, int *startpreset, int *endpreset)
{
...
void CHsvEuropeInstallation_mblizoosort_Priv::mReInnovateFromPreset(int ServiceType, int *FromPreset)
{
	int numdigservice;
...
Bool CHsvEuropeInstallation_mblizoosort_Priv::mTSInTransmission(void)
{
	Bool retval = FALSE;
	HsvPgdatDigTSData tsdat;
...
inline int CHsvEuropeInstallation_mblizoosort_Priv::ANALOG_STARTPRESET(void)
{
...
inline int CHsvEuropeInstallation_mblizoosort_Priv::NONRADIO_STARTPRESET(void)
{
	int retval, LastLCNPreset = mLastLCNBasedPreset(FALSE);
...
inline int CHsvEuropeInstallation_mblizoosort_Priv::RADIO_STARTPRESET(void)
{
...
int CHsvEuropeInstallation_mblizoosort_Priv::mGetAutomaticPlatformPresetNumber(void)
{
...
Bool CHsvEuropeInstallation_mblizoosort_Priv::mStaticSortingAvailable(Nat16 NetworkId)
{
	Bool retval = FALSE;
	Nat16 NetworkOperator = rins_GetAttribute(AUTO_MODE, rins_AttributeNetworkOperator);
...
Bool CHsvEuropeInstallation_mblizoosort_Priv::mGetValidRangeofPreset(int presetnum, int svcType)
{
	Bool retVal = FALSE;
...
/*Stub */
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AssignPresetsForCurrentCountryNonLCNs(int CurrentOnwid, int *FillHoles)
{
	return 0;
}
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AnalogPresetEntry(void)
{
	return 0;
}
void CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AssignPresetsForCountry(int ONID)
{
	return;
}
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_DeleteAllDigitalChannels()
{
	return 0;
}
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AssignPresetsForCurrentCountry  (int CurrentOnwid, Bool CheckLCN)
{
	return 0;
}
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AssignPresetsForCurrentCountryLowPrioLCN(int CurrentOnwid)
{
	return 0;
}
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AssignPresetsWithStaticTable ( int CurrentOnwid, int Country )
{
	return 0;
}
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_UpdatingLCNDuringUpdateInstallation()
{
	return 0;
}
Bool CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AssignPresetsOfOtherCountries   (void)
{
	return 0;
}
int CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_GetOriginalNetworkId(int CurrentCountry)
{
	return 0;
}
void CHsvEuropeInstallation_mblizoosort_Priv::iblizoosort_AddDigitalPreset(HsvPgdatDigServiceData *svcdat)
{
	return;
}
