...
#define	MAX_PGDB_TABLES				(12)

#define MAX_SAT_PGDB_TABLES			(10)
#define NUM_PGDB_SUBTABLES			(6)

#define	SQL_TABLE_LARGE_QUERY_LENGTH		(4*1024)

#define	SQL_TABLE_MEDIUM_QUERY_LENGTH		(2*1024)

#define	SQL_TABLE_SMALL_QUERY_LENGTH		(128)

#define MAX_DIRECTORY_NAME_SIZE         (100)

#define MAX_FILE_NAME_SIZE              (100)

#define	SQL_TEMP_STRING_LENGTH			(256)

#define SEM_TAKE(sem)                   rtk_SemAcquire(mProgramMapSem)

#define SEM_GIVE(sem)                   rtk_SemRelease(mProgramMapSem)
...
#define DATABASE_DIRECTORY_PATH		"/user_setting/rw/tvserver/databases/s2channellib/"
...
#define 	PERFORM_DATABASE_SYNC(DbToSync,FromTableId,ToTableId,Index,TotalNumberOfEntries,Data) 			TotalNumberOfEntries = srtdb2_GetNoOfRecords(FromTableId);\
		...
		{\
			...
			{\
				...
				{\
					...
					{\
						TraceNotice (m, "PERFORM_DATABASE_SYNC failed for Table %d Index %d ", DbToSync, Index);\
					}\
				}\
			}\
		}
...
Srtdbconfig	mProgramDbMap[NUM_PGDB_SUBTABLES];
static Srtdbconfig	s__mProgramDbMap[NUM_PGDB_SUBTABLES];
int	mActiveDatabase;
static int	s__mActiveDatabase;
HsvPgdatDigSatInfoData DefaultSatInfoValues;
static HsvPgdatDigSatInfoData s__DefaultSatInfoValues;
...
HsvPgdatHomServiceData DefaultHomingSvcValues;
static HsvPgdatHomServiceData s__DefaultHomingSvcValues;
HsvPgdatIntrServiceData DefaultInteractiveSvcValues;
static HsvPgdatIntrServiceData s__DefaultInteractiveSvcValues;
...
int GetDataFromTableByKeyForFreesat (int MajorTableId, int TableId, Address Data);
...
void module__init(void){
	...
	DefaultHomingSvcValues = s__DefaultHomingSvcValues;
	DefaultInteractiveSvcValues = s__DefaultInteractiveSvcValues;
