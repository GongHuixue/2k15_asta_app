#ifndef  _CHSVDVBSASTRA_MACQ_H
#define  _CHSVDVBSASTRA_MACQ_H 
...
typedef enum
{
	...
	AstraSDSDChange,
	AstraAcqModeChange
}AstraKeyUpdateReason;

typedef enum
{
	AstraHoming = 1,
	AstraAcquireServices
}AstraAcqMode;
...
typedef struct
{
	...
	HsvNITSatelliteDS SatDesc[ASTRA_MAX_SDSD];
}AstraSatDSDesc;
...
typedef struct
{
	...
	Nat8	LnbNumber;
	AstraAcqMode Mode;
} AstraDataAcqInstance;

class CHsvDvbsAstra_mAcq: private IHsvPower,IHsvDigitalAcquisition,public CS2MwBase
{
public:
...
private:
Pumpdefines(CHsvDvbsAstra_mAcq);

private:
	
AstraSatDSDesc mAstraDSDesc;
static AstraSatDSDesc s__mAstraDSDesc;
...
AstraDataAcqInstance DataAcqInstance;
static AstraDataAcqInstance s__DataAcqInstance;
...
virtual void AcquireDataOnTimeout();
virtual int GetNumberOfDigitalChannelsFound();
virtual int GetNumberOfDigitalChannelsRemoved();
virtual int GetNumberOfTvChannelsFound();
virtual int GetNumberOfRadioChannelsFound();
virtual int GetNumberOfTvChannelsRemoved();
virtual int GetNumberofRadioChannelsRemoved();
virtual int StoreBarkerMuxDetails();
virtual void SyncTemporaryTableRecords();
virtual void GetTricolorRegionNames (Nat16 **names, int *len);
virtual int GetNumberOfSdsds();
virtual Bool GetSdsdByIndex(int count,HsvNITSatelliteDS * SdSdEntry);
...
void mDeleteMuxesFromCurrentNIT();
Bool mIsTSPresentInSDSDList(HsvPgdatDigTSData *tmptsdata);
void mAddToTSTable(HsvInstallationPTC *ptc, AstraKeyUpdateReason *KeyUpdate, HsvPgdatDigTSData *OldTsData);
Bool mInstallMux(HsvInstallationPTC ptc, AstraKeyUpdateReason KeyUpdate, HsvPgdatDigTSData* OldTsData);
Bool mCheckServiceForInstall(HsvInstallationPTC *ptc, AstraKeyUpdateReason KeyUpdate, HsvPgdatDigTSData* OldTsData, HSVSPTYPE(ActualTSService, dvb, sgt) *service);
void mDeleteServicesCurrentCarrier(HsvInstallationPTC *ptc);
Bool mIsSvcPresentInSGT(HsvPgdatDigServiceData *svcdata);
void mDeleteAllServicesFromCarrier(int TPID);
void mUpdateService(HsvPgdatDigServiceData* psvcdat, HsvInstallationPTC* ptc,HSVSPTYPE(ActualTSService, dvb, sgt) *service);
void mUpdateSatelliteName (HsvPgdatDigServiceData *psvcdat);
void mAddNewService(HsvPgdatDigServiceData *psvcdat ,HsvInstallationPTC *ptc, HSVSPTYPE(ActualTSService, dvb, sgt) *service, Bool IsDuplicatesvc);
void mAssignSvcBroadcastAttributes(HsvPgdatDigServiceData *psvcdat, HsvInstallationPTC *ptc, HSVSPTYPE(ActualTSService, dvb, sgt) *service, Bool IsChannelExists);
...
Bool mIsServiceInstallable(HSVSPTYPE(ActualTSService, dvb, sgt)* service);
void mCollectSdsds ();
void mGetSatelliteLnbNo(int SatId);
Bool IsManualInstall();
Bool IsUpdateInstall();
...
Bool mIsServiceNameValid(Nat16* src);
...
#define pmp_PmpCreateEx( pen, func, replace, name ) m_pfac.PmpCreate(pen,&CHsvDvbsAstra_mAcq::func,replace,(char *)name,this)
