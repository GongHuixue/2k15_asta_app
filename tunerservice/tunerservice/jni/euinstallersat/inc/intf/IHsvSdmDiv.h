...
public:
	...
	virtual int DataCollectionTimeout(void)= 0;
	...
	virtual int MaxSimplePmtFilterSupported(void)= 0;
	virtual int MaxSimplePmtSupported(void)= 0;
...
public: \
    ...
    InterfaceParam<Comp,int> m_SdmWaitTime; \
    InterfaceParam<Comp,int> m_MaxStringLength; \
    InterfaceParam<Comp,int> m_DataCollectionTimeout; \
    ...
    InterfaceParam<Comp,int> m_MaxSimplePmtFilterSupported; \
    InterfaceParam<Comp,int> m_MaxSimplePmtSupported; \
    Comp ## _ ## intf( Comp *parent):m_NoOfDescriptors(parent),m_NoOfEvents(parent),m_SdmWaitTime(parent),m_MaxStringLength(parent),m_DataCollectionTimeout(parent),m_RRTParsingAllowed(parent),m_SystemTime(parent),m_RatingStringLength(parent),m_Enable12hourEitParsing(parent),m_MaxPmtMonitors(parent),m_ChannelNameLength(parent),m_SystemLanguage(parent),m_MaxSimplePmtFilterSupported(parent),m_MaxSimplePmtSupported(parent) \
    {\
        m_parent = parent;\
    }\
...
virtual int SdmWaitTime(void){ return m_SdmWaitTime.Call();}\
virtual int MaxStringLength(void){ return m_MaxStringLength.Call();}\
virtual int DataCollectionTimeout(void){ return m_DataCollectionTimeout.Call();}\
...
virtual int MaxSimplePmtFilterSupported(void){ return m_MaxSimplePmtFilterSupported.Call();}\
virtual int MaxSimplePmtSupported(void){ return m_MaxSimplePmtSupported.Call();}\
