...
public:
	...
	virtual int FreqMapTable(void)= 0;
...
public: \
    ...
    InterfaceParam<Comp,int> m_FreqMapTable; \
    InterfaceParam<Comp,int> m_CurrentDigSrvcTable; \
    InterfaceParam<Comp,int> m_CurrentDigTSTable; \
    Comp ## _ ## intf( Comp *parent):m_UserTable(parent),m_DigSrvcTable(parent),m_DigTSTable(parent),m_AnalogTable(parent),m_FreqMapTable(parent),m_CurrentDigSrvcTable(parent),m_CurrentDigTSTable(parent) \
    {\
        m_parent = parent;\
    }\
...
virtual int FreqMapTable(void){ return m_FreqMapTable.Call();}\
