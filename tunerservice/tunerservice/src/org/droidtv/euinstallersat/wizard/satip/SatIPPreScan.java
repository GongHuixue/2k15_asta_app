...
public class SatIPPreScan extends WizardStep implements SatelliteScreen
{

	private static final String TAG = SatIPPreScan.class.getName();
	...
	private OnClickListener mDoneButtonListener;
	
	private boolean ifSatIPServerFound = false;
	
	public SatIPPreScan(Context context, AttributeSet attrSet, int defStyle)
	{
	...
	public SatIPPreScan(Context context, AttributeSet attrSet)
	{
		this(context, attrSet, 0);
	}

	public SatIPPreScan(Context context)
	{
		this(context, null, 0);
	}
	...
	private void initButtonListeners()
	{
		...
		mDoneButtonListener = new OnClickListener(){
			@Override
			public void onClick(View arg0) {
				Log.d(TAG, "Pessed on Done Button");
	...
	private void onScanComplete() {
		...
		if (m_validLNBIdxArray.size() > 0) {
			...
			// satIP
			if (ifSatIPServerFound) {
				setButton3(m_context.getString(org.droidtv.ui.strings.R.string.MAIN_BUTTON_DONE), mDoneButtonListener, View.VISIBLE);
			} else {
				setButton3(m_context.getString(org.droidtv.ui.strings.R.string.MAIN_BUTTON_DONE), null, View.INVISIBLE);
				setHintText(m_context.getString(org.droidtv.tunerservice.R.string.MAIN_WI_SAT_IP_PRESCAN_FAILED));
			}
	...
	public void update(Observable observable, Object data)
	{
		...
		switch (eventID)
		{
			case EventIDs.PRESCAN_SUCCESS:
				...
				// satIP
				Log.d(TAG,"ifSatIPServerFound " + ifSatIPServerFound);

				((Activity) m_context).runOnUiThread(new Runnable() {
	
					@Override
					public void run() {
						if (ifSatIPServerFound == false) {
							// still Astra 19.2 not detected scan next LND
							scanNextLNB();
						} else {
							onScanComplete();
						}
			...
			case EventIDs.SATIP_NETWK_CONN_CHANGE:
				...
				if(m_wrapper.ifNetworkAvailable() == false){
					// show timed notification message
					ifSatIPServerFound = false;
					// reset found satellite
					swrap.setSatIPSatelliteInfo(0, 0);
					
					((Activity) m_context).runOnUiThread(new Runnable() {
						@Override
						public void run() {
							TvNotificationManager.postTvNotification(m_context,TvNotificationManager.FLAG_NO_IMAGE,
									m_context.getString(org.droidtv.tunerservice.R.string.MAIN_MSG_SAT_IP_PRESCAN_NOT_POSSIBLE),
	...
	private void updateLNBInfo(final int p_lnbIdx, final boolean p_isScanSuccesful)
	{
		...
		// Shiva: satIP chnage
		if( p_isScanSuccesful ){
			ifSatIPServerFound = true;
			
			// update installed satellite info
			swrap.setSatIPSatelliteInfo(p_lnbIdx, MwDataTypes.ASTRA_19_2_E);
		}else{
			// reset found satellite
			swrap.setSatIPSatelliteInfo(0, 0);
		}
		...
		l_activity.runOnUiThread(new Runnable()
		{
			...
			public void run()
			{
				if(p_isScanSuccesful)
				{
					...
					if(!ui_isDuplicate && ifSatIPServerFound)
					{
	...
	{
		return ScreenRequest.SATIP_PRESCAN;
	}
