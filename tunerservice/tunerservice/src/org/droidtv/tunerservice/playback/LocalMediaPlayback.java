...
public class LocalMediaPlayback extends TvAbstractPlayback implements OnSeekCompleteListener{
    private static final String TAG = "LocalMediaPlayback";
	...
	public static final String SONG_TYPE 	= "audio";
	...
	public LocalMediaPlayback(Context context, PlayTVFacMediaPlayer mpl) {
	...
	public void play(Uri uri, Medium medium, Map<String,String> headers) {
		...
		int isPartial = 0;
		String file_uri = headers.get(TvHeaders.FILE_URI) + "?mimeType=0&tvUriType=20&bcstTS=0";
		String imagepath = headers.get(TvHeaders.FILE_URI);
		String file_uri2 = headers.get(TvHeaders.FILE_URI);
		...
		if (!tvserverPlayTriggered) {
			...
			trickmodeStatus		= true;
				...
				if(IsImageFormat(Uri.parse(file_uri))){
					playtvmpl.PlayLocalFile(Uri.parse(file_uri), headers , true);
				}else{
					playtvmpl.PlayLocalFile(Uri.parse(file_uri), headers , false);
				}
	...
	private void addView(Uri uri) {
		...
		Log.d(TAG, "[HTV]666666666 play PlayLocalFile is 1");
		//picView.setImageDrawable(loadImageFromURL(uri.toString()));
		Log.d(TAG, "[HTV]888888888888888 play PlayLocalFile is 1");
	}
	
	private Drawable loadImageFromURL(String url){
    	try{
			Log.d(TAG, "[HTV]777777777 play PlayLocalFile is 1");
        	Drawable draw = Drawable.createFromPath(url);
        	return draw;
    	}catch (Exception e) {
        	//TODO handle error
   ...
   private class SeekDoneRunnable implements Runnable {
	...
	public void onPlayStarted(Uri uri, Map<String,String> headers, Medium medium, int presetNumber) {
		...
		PlaybackState = ITvAbstractPlaybackListener.PLAYBACK_STATE_STOPPED;
		onPlayStateChanged(PlaybackState);
		Intent intent = new Intent("org.droidtv.localmedia.onPlayStarted");
	...
	public void onPlayCompleted(Uri uri, Map<String,String> headers, Medium medium, int presetNumber) {
		...
		Intent intent = new Intent("org.droidtv.localmedia.onPlayCompleted");
		mContext.sendBroadcast(intent);
	}
	...
	Log.e(TAG, "onPlayFailed ");
		...
		Intent intent = new Intent("org.droidtv.localmedia.onPlayFailed");
		mContext.sendBroadcast(intent);
	}
...
public void HandleNotPlayable(int starttime,int direction) {
	...
	private boolean IsImageFormat(Uri uri){
		//FileTypeEnum type = getFileType(getUriMIMEType(uri));

		String Uri = uri.toString();

		Log.d(TAG, "IsImageFormat: "+Uri);
		if(	Uri.contains(".bmp") ||
			Uri.contains(".gif") ||
			Uri.contains(".jpg") ||
			Uri.contains(".jpeg") ||
	...
	public enum FileTypeEnum {
		...
		public static FileTypeEnum valueOf(int val) {
	...
	public class PlaytvStateChangeCallbacks implements IPlayTvStateChangeListener {
		...
		public void onPlayStarted(Uri mediaPlayerUri,  Map<String,String> headers, Medium medium, int presetNumber) {
			LocalMediaPlayback.this.onPlayStarted(mediaPlayerUri, headers, medium, presetNumber);
		}

		@Override
		public void onPlayCompleted(Uri mediaPlayerUri, Map<String,String> headers, Medium medium, int presetNumber) {
			LocalMediaPlayback.this.onPlayCompleted(mediaPlayerUri, headers, medium, presetNumber);
		}

		@Override
		public void onPlayFinished(Uri mediaPlayerUri, Medium medium, int presetNumber) {
			LocalMediaPlayback.this.onPlayFinished(medium, presetNumber);
		}

		@Override
		public void onPlayFailed(Uri mediaPlayerUri, Medium medium, int presetNumber, int reason) {
			LocalMediaPlayback.this.onPlayFailed(mediaPlayerUri, medium, presetNumber, reason);
		}

		@Override
		public void onInfo(boolean cicamAvailable) {
			LocalMediaPlayback.this.onInfo(cicamAvailable);
		}

		@Override
		public void onError(PlayTvMediaPlayer mpl, int what , int extra) {
			LocalMediaPlayback.this.onError(mpl, what, extra);
		}

		@Override
		public void onLockStatusChanged(int type, int status) {
			LocalMediaPlayback.this.onLockStatusChanged(type, status);
		}

		@Override
		public void onPurposeChanged(int purpose) {
			LocalMediaPlayback.this.onMPLPurposeChanged(purpose);
		}
