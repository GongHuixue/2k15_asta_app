#ifndef  _CHSVDVBCAMINSTALLATION_MSCAN_PRIV_H
#define  _CHSVDVBCAMINSTALLATION_MSCAN_PRIV_H 
...
#define CLASSSCOPE CHsvDvbCamInstallation_mScan_Priv::
#include "locals_mScan.h"

class CHsvDvbCamInstallation_mScan_Priv : public CTCMwBase
{
public:
virtual ~CHsvDvbCamInstallation_mScan_Priv();
...
//Provided interfaces
private:
IEnableImpl(CHsvDvbCamInstallation_mScan_Priv,ena);
IHsvFrontEndApiNotifyImpl(CHsvDvbCamInstallation_mScan_Priv,feapiN);
IHsvCamScanHelperImpl(CHsvDvbCamInstallation_mScan_Priv,icamhelper);
IHsvDigitalScanImpl(CHsvDvbCamInstallation_mScan_Priv,iins);
ICesOperatorProfileNotifyImpl(CHsvDvbCamInstallation_mScan_Priv,opinsN);
IHsvPowerImpl(CHsvDvbCamInstallation_mScan_Priv,pow);
IHsvStreamNotifyImpl(CHsvDvbCamInstallation_mScan_Priv,strapiN);
public:
...
private:
Pumpdefines(CHsvDvbCamInstallation_mScan_Priv);

public:
CHsvDvbCamInstallation_mScan_Priv():i__ena(this),i__feapiN(this),i__icamhelper(this),i__iins(this),i__opinsN(this),i__pow(this),i__strapiN(this)
{
ena	=	&i__ena;
feapiN	=	&i__feapiN;
icamhelper	=	&i__icamhelper;
iins	=	&i__iins;
opinsN	=	&i__opinsN;
pow	=	&i__pow;
strapiN	=	&i__strapiN;
...
#define camScanparser_iPresent() (camScanparser.Present())
#define camScanparser_CamNitParser_Parser(dest,cookie,DataSize,DataPtr)	camScanparser->CamNitParser_Parser(dest,cookie,DataSize,DataPtr)
#define camScanparser_GetCamInstallationMode()	camScanparser->GetCamInstallationMode()
#define camScanparser_SetCharacterCode(charCode)	camScanparser->SetCharacterCode(charCode)
#define camacqhelper_iPresent() (camacqhelper.Present())
#define camacqhelper_CamNitParser_Parser(dest,cookie,DataSize,DataPtr)	camacqhelper->CamNitParser_Parser(dest,cookie,DataSize,DataPtr)
#define camacqhelper_GetCamInstallationMode()	camacqhelper->GetCamInstallationMode()
#define camacqhelper_SetCharacterCode(charCode)	camacqhelper->SetCharacterCode(charCode)
...
#define camena_iPresent() (camena.Present())
#define camena_Enable(dmx)	camena->Enable(dmx)
#define camena_Disable(dmx)	camena->Disable(dmx)
#define camena_Sync(dmx)	camena->Sync(dmx)
...
#define chdecdvbt2_GetCarrierPresent(winid,Present)	chdecdvbt2->GetCarrierPresent(winid,Present)
...
#define chdecdvbt2_GetSuppChanBandwidths(winid,Bandwidths)	chdecdvbt2->GetSuppChanBandwidths(winid,Bandwidths)
#define chdecdvbt2_SetChanBandwidth(winid,bandwidth)	chdecdvbt2->SetChanBandwidth(winid,bandwidth)
#define chdecdvbt2_GetChanBandwidth(winid,Bandwidth)	chdecdvbt2->GetChanBandwidth(winid,Bandwidth)
#define chdecdvbt2_GetActualChanBandwidth(winid,Bandwidth)	chdecdvbt2->GetActualChanBandwidth(winid,Bandwidth)
#define chdecdvbt2_GetAdvancedSignalPresence(winid,advsig)	chdecdvbt2->GetAdvancedSignalPresence(winid,advsig)
#define chdecdvbt2_GetBer(winid,Ber,retval)	chdecdvbt2->GetBer(winid,Ber,retval)
...
#define insN_EventCamInstallRequestUrgent	IHsvInstallationNotify_EventCamInstallRequestUrgent
#define insN_EventCamInstallRequestNormal	IHsvInstallationNotify_EventCamInstallRequestNormal
...
#define opins_iPresent() (opins.Present())
#define opins_RequestStatusInfo()	opins->RequestStatusInfo()
#define opins_RequestNit()	opins->RequestNit()
#define opins_SendOperatorSearchStart(searchstart)	opins->SendOperatorSearchStart(searchstart)
#define opins_SendOpExit()	opins->SendOpExit()
#define opins_SendOpTuneStatus(tuneStatus)	opins->SendOpTuneStatus(tuneStatus)
#define opins_SendOpAck()	opins->SendOpAck()
#define opins_SendOpSearchCancel()	opins->SendOpSearchCancel()
#define opins_CacheOperatorData()	opins->CacheOperatorData()
...
#define pmp_PmpCreateEx( pen, func, replace, name ) m_pfac.PmpCreate(pen,&CHsvDvbCamInstallation_mScan_Priv::func,replace,(char *)name,this)
