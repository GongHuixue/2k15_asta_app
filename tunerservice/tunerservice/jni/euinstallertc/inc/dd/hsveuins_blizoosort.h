...
#define IS_SYSTEM_NETWORKOPERATOR_Blizoo							(((sysset_GetSystemCountry() == cids_CountryBulgaria) )&&(rins_GetAttribute(rins_InstallationModeAutomatic, rins_AttributeNetworkOperator) == rins_Blizoo)? true : false)
...
typedef enum
{
    evServiceIDSorting = 0,
    evAnalogSorting,
    evComplete,
    evNoSorting,
}SortSteps;

static Nat16 mGermanyKDGRankingList[] = 
{
#define SVC_ITEM(SVC_Name, SVC_Id)  SVC_Id
#include "GermanyKDGRankingList.h"
#undef  SVC_ITEM
...
static int  mCurrentTvPreset = INVALID_PRESET, mCurrentRadioPreset = INVALID_PRESET, mFirstInstalledPreset = INVALID_PRESET,mCurrentPreset_InValidLCN_Blizoo = 800;
static int  mCurrentBlizooPreset; 


/* module functions */
static void mSwap(int* a, int* b) ;
static void mMaxHeapify( MaxHeap* maxHeap, int idx);
static  MaxHeap* mCreateAndBuildHeap(BlizooSort* array, int size);
static void mHeapSort(BlizooSort* array, int size);
static void mBlizoosortHandler(int value, Nat32 param);
static void mSortAnalogServices(int evCurrentSortStep);
static void mSortHonouredServices(Bool CheckForRadioServices, int evCurrentSortStep);
static void mSortBlizooServices( int evCurrentSortStep);
static void mSortNonHonouredServices(Bool CheckForRadioServices, int evCurrentSortStep);
