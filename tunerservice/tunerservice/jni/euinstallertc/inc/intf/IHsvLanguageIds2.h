...
public:
	...
	#define IHsvLanguageIds2_LangBahasaMelayu       ((int)48)
	#define IHsvLanguageIds2_LangTraditionalChinese ((int)49)
	#define IHsvLanguageIds2_LangNone		((int)50 )
	virtual int MaxNumberOfLanguages(void)= 0;
...
private:\
    Comp *m_parent; \
public: \
    InterfaceParam<Comp,int> m_MaxNumberOfLanguages; \
    InterfaceParam<Comp,int> m_iVersion; \
    Comp ## _ ## intf( Comp *parent):m_MaxNumberOfLanguages(parent),m_iVersion(parent) \
    {\
        m_parent = parent;\
    }\
virtual int MaxNumberOfLanguages(void){ return m_MaxNumberOfLanguages.Call();}\
